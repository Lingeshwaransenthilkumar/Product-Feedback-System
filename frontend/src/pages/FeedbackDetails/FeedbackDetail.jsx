...
export default function FeedbackDetails() {
  ...
  const handleAddComment = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/feedbacks/${id}/comment`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userName: "GuestUser", text: commentText }),
        }
      );
      const data = await res.json();
      setFeedback(data);
  ...
  const handleReply = async (commentId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/feedbacks/${id}/comment/${commentId}/reply`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userName: "GuestUser", text: replyText }),
        }
      );
      const data = await res.json();
      setFeedback(data);
