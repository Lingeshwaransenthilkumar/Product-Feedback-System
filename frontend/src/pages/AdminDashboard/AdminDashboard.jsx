...
export default function AdminDashboard() {
  ...
  const fetchUsersCount = async () => {
    try {
      let res = await fetch("http://localhost:5000/api/users/count");
      if (res.ok) {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const res = await fetch(`http://localhost:5000/api/feedbacks/${id}/status`, {
        method: "PATCH",
    ...
    <div className="admin-wrap">
      <header className="admin-header">
        ...
        <div className="admin-actions">
          <div className="greeting">👋 Hi, {localStorage.getItem("name") || "Admin"}</div>
